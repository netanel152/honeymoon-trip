...
## כפתורי Uber ליד כל יעד (4.9.2026)

הבעלים ביקש כפתורי Uber/Bolt ליד היעדים בלוז היומי. **Bolt לא פועלת בטאיוואן וביפן**, אז
נוסף Uber בלבד (ביפן Uber מזמינה מונית רגילה; ב-GO אין קישור עומק מתועד).

- שורות ה-`.tline` הן טקסט חופשי בלי יעד מובנה, לכן הכפתורים יושבים על **כל `a.place`** —
  בבלוק "🗺️ כל המקומות של היום" של 36 הימים, ב-`pane-places` ובמקומות משלנו (769 כרטיסים).
  הכל **מוזרק בזמן ריצה** בסקריפט קטן בסוף ה-body, מהקישור הקיים לגוגל מפות (`query=` או
  `/maps/place/<שם>`), אז אין כתובות לתחזק. `MutationObserver` על `#pane-places` מעטר מחדש
  אחרי שהמקומות שלנו מרונדרים ב-`innerHTML`.
- הקישור: `https://m.uber.com/ul/?action=setPickup&pickup=my_location&dropoff[nickname]=…&dropoff[formatted_address]=…`.
  בפאנל היום נוסף "🚕 Uber למלון" עם `dropoff[latitude]/[longitude]` מדויקים מ-`MAP_STOPS`.
- **הכפתור הוא `<a>` בתוך `a.place`** — HTML לא תקני, אבל מוזרק דרך ה-DOM (לא innerHTML) ולכן
  עובד: נבדק ב-Playwright שלחיצה על הכפתור פותחת רק את Uber ולחיצה על הכרטיס רק את Maps.
  אין לרנדר אותו בתוך מחרוזת innerHTML — הפרסר יפצל את ה-`<a>` המקונן.
- הטקסט "Uber" מגיע מ-`::after` (ואייקון מ-`::before`) כדי ש-`textContent` יישאר נקי —
  אחרת חיפוש "uber" ב-`pane-places` היה מתאים לכל 475 המקומות. נבדק: "uber" → 0 תוצאות.
- **בנייד (≤560px) האייקון בלבד**: כרטיס מקום מכיל ~266px תוכן, והמילה Uber דחקה שמות
  ארוכים לשלוש שורות. ה-`aria-label` והערת `.ride-note` מעל כל רשימה מסבירים.
- `.td-ride` בפאנל היום: `align-self:flex-start` חובה — `.td-box` הוא flex-column והכפתור נמתח
  לרוחב מלא בלעדיו. הפאנל כהה בשני המצבים, אז צבעיו קבועים ולא ב-dark block.
